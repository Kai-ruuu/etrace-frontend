<script>
    ...
    async function archive(course) {
        await Course.archiveById(course.id)

        const pageIsEmpty = coursesInfo.items.length - 1 === 0
        
        await refetchHandler(pageIsEmpty)
    }
    
    async function restore(course) {
        await Course.restoreById(course.id)
...
<div class="px-8 bg-white space-y-6">
    ...
    <div class="overflow-x-hidden overflow-y-auto">
        {#if coursesInfo.loading}
            <LoadingDataIndicator>Loading Courses...</LoadingDataIndicator>
        {:else}
            <table>
                <thead>
                    <tr>
                ...
                <tbody>
                    {#each coursesInfo.items as course}
                        <tr>
                            ...
                            <td class="p-2">
                                <Button
                                    variant="sm"
                                    onclick={async () => {
                                        if (forArchived) await restore(course)
                                        else await archive(course)
                                    }}
                                >
                                    {#if forArchived}
...
<Paginator {fetchPrevHandler} {fetchNextHandler} info={coursesInfo} />